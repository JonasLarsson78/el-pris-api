...
elRoute.get('/el', async (req, res) => {
  ...
  const monthDate = monthNow.toString().padStart(2, '0') + '-' + dateNow.toString().padStart(2, '0')
  const yearNow = now.getFullYear()?.toString()
  
  const { year = yearNow, date = monthDate, zone = 'SE4'} = req.query
  
  try {
    const response = await fetch(`https://www.elprisetjustnu.se/api/v1/prices/${year}/${date}_${zone}.json`)
